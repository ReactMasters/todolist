...
import { CreateTodoGroupOutput } from './dto/create-todo-group.output'
...
export class TodoGroupResolver {
  ...
  async createTodoGroup(
    @Args('createTodoGroupInput') createTodoGroupInput: CreateTodoGroupInput
  ) {
    const res: CreateTodoGroupOutput = {
      success: true,
    }
    try {
      res.todoGroup = await this.todoGroupService.createTodoGroup(
